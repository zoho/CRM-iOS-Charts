...
public class ZCRMKPI :UIView {
	...
	public var comparedTo: String!  {
		didSet {
			if comparedTo != nil && self.type != .rankings && self.type != .basic {
				self.setComparedToViewText(text: comparedTo)
			}
		}
	}
	...
	private var _data: [ZCRMKPIRow] = []
	
	private let comparedToView = UILabel() // for standard, growth index and scorecard
	private let valueView = UILabel() //for standard, growth index and basic
	...
	*/
	private func render() {
		...
		self.layoutIfNeeded()
//		self.backgroundColor = UIColor(red: 182/255.0, green: 186/255.0, blue: 193/255.0, alpha: 1) // MARK: - to change
	...
	*/
	...
	/**
		Sets the title text to the title view. It will be common for all types.
	*/
	private func setUpTitleText(fontSize: CGFloat = 17, color: UIColor = UIColor.white) {
		
		let size = (self.type == .rankings || self.type == .scorecard) ? 18 : fontSize
		var font: UIFont!
		...
		self.titleView.attributedText = NSAttributedString(string: self.title, attributes: [ .font : font, .foregroundColor : color])
	}
	...
	*/
	private func setComparedToViewText(text: String, fontSize: CGFloat = 12, color: UIColor = UIColor.white) {
		
		if self.type == .standard || self.type == .growthIndex {
			self.comparedToView.textAlignment = .right
		} else {
			self.comparedToView.textAlignment = .left
		}
		self.comparedToView.attributedText = NSAttributedString(string: text, attributes: [.font : UIFont.systemFont(ofSize: fontSize), .foregroundColor : color])
	}
	...
	*/
	private func setSimpleKpiData() {
		...
		let data = self._data[0]
		if self.type == .standard {
			self.valueView.attributedText = ZCRMKPIUIUtil.shared.getValueTextForStandardKPI(data: data)
		} else if self.type == .growthIndex {
			self.valueView.attributedText = ZCRMKPIUIUtil.shared.getValueTextForGrowthIndexKPI(data: data)
		} else if self.type == .basic {
			self.valueView.attributedText = ZCRMKPIUIUtil.shared.getValueTextForBasicKPI(data: data)
		}
	}
	
	/**
		Sets the view for scorecard KPI.
	*/
	...
	*/
	private func addKpiConstraints() {
		...
		if tVHeight  > (CGFloat(self._data.count) * cellHeight )  + self.getHeightOf(percent: 2){
			tVHeight = (CGFloat(self._data.count) * cellHeight ) + self.getHeightOf(percent: 2)
		}
...
extension ZCRMKPI : UITableViewDataSource, UITableViewDelegate {
	...
	public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		if self.type == .rankings {
			cell = ZCRMKPICell(data:  self._data[indexPath.row], type: self.type, highRate: self._data[0].difference.toCGFloat())
		} else {
			 cell = ZCRMKPICell(data: self._data[indexPath.row], type: self.type)
		}
